...
```bash
(subfinder -d target.com -silent; assetfinder --subs-only target.com; amass enum -passive -d target.com) | sort -u
```
...
Mass scan semua subdomain:

```bash
cat live.txt | xargs -I{} ffuf -u {}/FUZZ -w /usr/share/wordlists/dirb/common.txt -mc 200
```
...
```bash
cat live.txt | xargs -I{} curl -s {}/.env
```
